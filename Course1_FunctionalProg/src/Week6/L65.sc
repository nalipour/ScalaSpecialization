...
object L65 {
  println("Welcome to the Scala worksheet")       //> Welcome to the Scala worksheet
	...
	val in = Source.fromFile("/Users/nalipour/Documents/courses/ScalaSpecialization/Course1_FunctionalProg/src/Week6/linuxwords.txt")
	...
	val wordsForNum: Map[String, Seq[String]] =
		words groupBy wordCode withDefaultValue Seq()
                                                  ...
                                                  //| -> List(evolve), 78475464 -> List(Stirling), 746459 -> List(singly), 5466377
                                                  //| 37 -> List(lionesses), 28754283 -> List(curlicue), 84863372658 -> List(thund
                                                  //| erbolt), 46767833 -> List(imported), 26437464 -> List(angering, cohering), 8
                                                  //| 872267 -> List(turbans), 77665377 -> List(spoolers), 46636233 -> List(homema
                                                  //| de), 7446768759 -> List(rigorously), 74644647 -> List(ringings), 633738 -> L
                                                  //| ist(offset), 772832 -> List(Pravda), 4729378 -> List(grayest), 894824464 -> 
                                                  //| List(twitching), 8622443
                                                  //| Output exceeds cutoff limit.
